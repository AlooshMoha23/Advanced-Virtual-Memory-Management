...
int PhysicalMemManager::AddPhysicalToVirtualMapping(AddrSpace *owner,
  ...
  printf("**** Warning: function AddPhysicalToVirtualMapping is not "
         "implemented\n");
  exit(ERROR);
  return (0);
}
...
int PhysicalMemManager::AddPhysicalToVirtualMapping(AddrSpace *owner,
  ...
  int physicalPage = g_physical_mem_manager->FindFreePage();
  if (physicalPage == INVALID_PAGE) {
    // No free physical pages, need to implement page replacement
    physicalPage = g_physical_mem_manager->EvictPage();
    if (physicalPage == INVALID_PAGE) {
      printf("*********************************************\n");
      return INVALID_PAGE;
    }
  } else {
    tpr[physicalPage].locked = true;
  }

  owner->translationTable->setPhysicalPage(virtualPage, physicalPage);
  // Update the real page table
  tpr[physicalPage].free = false;
  tpr[physicalPage].virtualPage = virtualPage;
  tpr[physicalPage].owner = owner;
...
int PhysicalMemManager::EvictPage() {
  ...
  while (true) {
    ...
    if (!tpr[i_clock].locked) {
      if (tpr[i_clock].owner->translationTable->getBitU(
              tpr[i_clock].virtualPage)) {
        tpr[i_clock].owner->translationTable->clearBitU(
            tpr[i_clock].virtualPage);
      } else {

        int physicalPage = i_clock;
        tpr[physicalPage].locked = true;
        tpr[physicalPage].owner->translationTable->clearBitValid(
            tpr[physicalPage].virtualPage);

        // Check the M bit before writing to disk
        if (tpr[physicalPage].owner->translationTable->getBitM(
                tpr[physicalPage].virtualPage)) {
          // Save the page to disk if it's not free and has been modified
          g_swap_manager->PutPageSwap(
              tpr[physicalPage].owner->translationTable->getAddrDisk(
                  tpr[physicalPage].virtualPage),
              (char *)&(g_machine->mainMemory[physicalPage * g_cfg->PageSize]));

          // Set the swap bit
          // tpr[physicalPage].owner->translationTable->setBitSwap(tpr[physicalPage].virtualPage);
        }
