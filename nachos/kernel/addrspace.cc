  ...
  *err = 0;
  ...
  for (int i = 0; i < elff.getShNum(); i++) {
    ...
    for (unsigned int pgdisk = 0,
      ...
      translationTable->setBitWriteAllowed(virt_page);
      translationTable->clearBitSwap(virt_page);
      translationTable->setAddrDisk(virt_page, elff.getShOffset(i) +
...
#define STACK_BLANK_LEN 4 // in pages
  ...
  // Print address range for stack even in non debug mode to help
  // debugging in case of stack overflow
...
#ifndef ETUDIANTS_TP
    /* Without demand paging */
    // Allocate a new physical page for the stack, halt if not page
    // available
    int pp = g_physical_mem_manager->FindFreePage();
    if (pp == INVALID_PAGE) {
 ...
 * \param size: size to be mapped in bytes (rounded up to next page
 * boundary) \return the virtual address at which the file is mapped
 */
// ----------------------------------------------------------------------
