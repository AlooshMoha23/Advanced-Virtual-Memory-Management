...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr) {
  ...
  DEBUG('e',
        (char *)"\033[1;31m EXCEPTION_HANDLER : \n\t\texceptionType = "
                "%d\n\t\tmem[17] = %d\n",
        exceptiontype, type);
  // exit(ERROR);
  ...
  case SYSCALL_EXCEPTION:
    ...
    case SC_EXEC: {
      ...
      DEBUG('e', (char *)"After Process created\n");
      ...
      DEBUG('e', (char *)"After Thread created (and\n");
      if (error != NO_ERROR) {
        g_machine->WriteIntRegister(10, ERROR);
        if (error == OUT_OF_MEMORY)
          g_syscall_error->SetMsg((char *)"", error);
    ...
    case SC_NEW_THREAD: {
      ...
      DEBUG('e', (char *)"SC_NEW_THREAD  fun=0x%x    arg=0x%x\n", fun, arg);
    ...
    case SC_JOIN: {
      ...
      {
        DEBUG('e', (char *)"JOIN : Thread specified not found tid=%d\n", tid);
        g_syscall_error->SetMsg((char *)"", NO_ERROR);
        g_machine->WriteIntRegister(10, NO_ERROR);
      }
...
#ifdef ETUDIANTS_TP
    case SC_P: {
      int32_t id = g_machine->ReadIntRegister(10);
      Semaphore *sem = (Semaphore *)g_object_addrs->SearchObject(id);
      ASSERT(sem);
      ASSERT(sem->type == SEMAPHORE_TYPE);
      sem->P();
      g_machine->WriteIntRegister(10, NO_ERROR);
      break;
    }
    ...
    case SC_SEM_CREATE: {
      int addr, size, initial_value;
      addr = g_machine->ReadIntRegister(10);
      size = GetLengthParam(addr);
      initial_value = g_machine->ReadIntRegister(11);
      char name[size];
      GetStringParam(addr, name, size);
      Semaphore *sem = new Semaphore((char *)name, initial_value);
      if (sem == NULL)
        g_machine->WriteIntRegister(10, ERROR);
      else
        g_machine->WriteIntRegister(10, g_object_addrs->AddObject(sem));
      break;
    }
    case SC_SEM_DESTROY: {
      int32_t id = g_machine->ReadIntRegister(10);
    ...
    case SC_LOCK_CREATE: {
      ...
      char name[size];
      GetStringParam(addr, name, size);
      Lock *lock = new Lock((char *)name);
      if (lock == NULL)
        g_machine->WriteIntRegister(10, ERROR);
      else
        g_machine->WriteIntRegister(10, g_object_addrs->AddObject(lock));
      break;
    }
    ...
    case SC_COND_CREATE: {
      ...
      char name[size];
      GetStringParam(addr, name, size);
      Condition *cond = new Condition((char *)name);
      if (cond == NULL)
        g_machine->WriteIntRegister(10, ERROR);
      else
        g_machine->WriteIntRegister(10, g_object_addrs->AddObject(cond));
      break;
    }
    case SC_COND_DESTROY: {
      ...
      ASSERT(cond);
      ASSERT(cond->type == CONDITION_TYPE);
      delete cond;
      g_machine->WriteIntRegister(10, NO_ERROR);
      break;
    }
