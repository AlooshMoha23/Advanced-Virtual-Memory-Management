...
void Semaphore::P() {
  ...
  // Compare to 0 :
  while (counter == 0) {
    DEBUG('s', (char *)"P() thread put to sleep\n");
    waiting_queue->Append((void *)g_current_thread);
    g_current_thread->Sleep();
    DEBUG('s', (char *)"P() thread awaken\n");
  }
...
void Semaphore::V() {
  ...
  // Compare to 0 :
  if (!waiting_queue->IsEmpty()) {
    DEBUG('s', (char *)"V() free the top thread\n");
...
#ifdef ETUDIANTS_TP
void Lock::Acquire() {
  DEBUG('s', (char *)"Acquire() called\n");

  // Disable interrupts :
  IntStatus prev = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);

  // Check wether lock is already acquired or not
  while (!free) {
    waiting_queue->Append((void *)g_current_thread);
    g_current_thread->Sleep();
  }
...
void Lock::Release() {
  ...
  // Free the first thread waiting in the queue
  if (!waiting_queue->IsEmpty()) {
    g_scheduler->ReadyToRun((Thread *)waiting_queue->Remove());
  }
  // Reset owner
...
void Condition::Signal() {
  ...
  if (!waiting_queue->IsEmpty()) {
    DEBUG('s', (char *)"Signal() pop thread\n");
    g_scheduler->ReadyToRun((Thread *)waiting_queue->Remove());
  } else
    DEBUG('s', (char *)"Signal() nothing to do\n");

  // Set status back
  g_machine->interrupt->SetStatus(prev);
  DEBUG('s', (char *)"After setting interrupts back\n");
}
#endif
...
#ifdef ETUDIANTS_TP
void Condition::Broadcast() {
  DEBUG('s', (char *)"Broadcast() called\n");

  // Disable interrupts :
  IntStatus prev = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);

  // Pop first thread from queue while not empty
