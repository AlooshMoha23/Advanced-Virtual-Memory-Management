...
int Thread::Start(Process *owner, int64_t func, int64_t arg) {
  ...
  // Put the process as owner of this thread
  process = owner;

  // Init stackPointer
  int stackPointer = owner->addrspace->StackAllocate();
  DEBUG('t', (char *)"ptr=%x\n", stackPointer);

  if (stackPointer == OUT_OF_MEMORY) {

    return OUT_OF_MEMORY;
  }

  DEBUG('t', "Start() name=\"%s\"   func=0x%x   sp=0x%x   arg=0x%x\n", name,
        func, stackPointer, arg);

  // Init both contexts
  InitThreadContext(func, stackPointer, arg);
  InitSimulatorContext(AllocBoundedArray(SIMULATORSTACKSIZE),
                       SIMULATORSTACKSIZE);

  // Add thread to list of alive threads
  g_alive->Append(this);

  // Set as ready to run
  g_scheduler->ReadyToRun(this);

  // Add 1 to thread count of the owner process
...
*/
//----------------------------------------------------------------------
void Thread::InitThreadContext(int64_t initialPCREG, int64_t initialSP,
                               int64_t arg) {
  int i;
...
void Thread::Finish() {
  // Disable interrupts :
  IntStatus prev = g_machine->interrupt->SetStatus(INTERRUPTS_OFF);

  DEBUG('t', (char *)"Finishing thread \"%s\"\n", GetName());

  // Sets to be destroyed
  g_thread_to_be_destroyed = this;

  // Remove from alive threads
  ...
  // Set status back
  g_machine->interrupt->SetStatus(prev);
}
#endif
...
#ifdef ETUDIANTS_TP
void Thread::SaveProcessorState() {
  DEBUG('t', (char *)"***SaveProcessorState \"%s\"\n", GetName());
...
void Thread::RestoreProcessorState() {
  ...
  DEBUG('t', (char *)"***RestoreProcessorState \"%s\"\n", GetName());

  // Restore the saved translation table
  g_machine->mmu->translationTable =
      g_current_thread->GetProcessOwner()->addrspace->translationTable;

  // Restore the user program's CPU state
  for (int i = 0; i < NUM_INT_REGS; i++) {
